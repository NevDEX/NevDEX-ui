            ...
            <div class="h-12 bg-transparent items-center flex text-gray-200 text-sm border-b border-gray-700 my-2">
              <ul class="flex justify-center items-center mx-auto space-x-4 flex-wrap w-full gap-1">
                <li v-for="(token, index) in tokenList" :key="index" class="mr-2">
                  <a href="javascript:void(0)" class="inline-block rounded-t-lg border-b-2 hover:text-gray-300 hover:border-blue-600 border-transparent" @click="toggleTabs(token)" v-bind:class="{ 'text-bold text-gray-300 border-blue-600 ': quoteToken === token }">{{ token }}</a>
                </li>
              </ul>
            </div>

            <MenuItem v-for="item in marketList[quoteToken]" :key="item.id" v-slot="{ active }" class="flex flex-row justify-center">
